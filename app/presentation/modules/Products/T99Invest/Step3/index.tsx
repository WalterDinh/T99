...
const index = (props: IProps) => {
    ...
    const { dataAssetInfo, setLoanFee } = useContext(T99InvestContext);
    ...
    const [initialValues, setInitialValues] = useState({
        loan: '',
        unit:
            dataAssetInfo.assetName.paymentWays == 1
                ? dataUnit[0]
                : dataUnit[1],
        period: '',
    ...
    const dataPeridod = (dataAssetInfo.assetName?.dataPeridod || []).map((item) => {
        return {
            key: item,
            label: item,
        };
